<x-updatecomponent title="{{ __('dashboard.update-role') }}">
    <div class="row">
                    ...
                    @foreach (config('permessions_ar') as $key => $value)
                        <div class="col-md-4 mb-2">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="perm_update_{{ $key }}"
                                    value="{{ $key }}" wire:model="permession">
                                <label class="form-check-label" for="perm_update_{{ $key }}">
                                    {{ $value }}
                                </label>
                            </div>
                        </div>
