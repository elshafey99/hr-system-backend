        ...
        <table class="table table-hover table-bordered">
            <thead class="table-light">
                <tr>
                    ...
                    <th class="align-middle" style="width: 10%;">{{ __('dashboard.code') }}</th>
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.phone-code') }}</th>
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.currency_code') }}</th>
                    <th class="text-center align-middle" style="width: 35%;">{{ __('dashboard.actions') }}</th>
                </tr>
            </thead>
            <tbody>
                @if ($data->count() > 0)
                    @foreach ($data as $item)
                        <tr wire:key="country-{{ $item->id }}" style="height: 80px;">
                            ...
                            <td class="align-middle text-center">
                                @if ($item->image)
                                    <img src="{{ asset($item->image) }}" alt="country image"
                            ...
                            <td class="align-middle">
                                <span class="text-muted">{{ $item->phone_code ?? '-' }}</span>
                            </td>
                            <td class="align-middle">
                                <span class="badge rounded-pill bg-success">{{ $item->currency_code }}</span>
                            </td>
                            <td class="text-center align-middle">
                                <div class="d-flex justify-content-center align-items-center gap-1">
                                    <button type="button"
                                        onclick="Livewire.dispatch('countryUpdate', { id: {{ $item->id }} })"
                                    ...
                                    <a href="{{ route('dashboard.governorates.index', ['country_id' => $item->id]) }}"
                                        title="{{ __('dashboard.governorates') }}"
